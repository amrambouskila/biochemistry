...
- **Container scanning** — wired: `aquasecurity/trivy-action@ed142fd0673e97e23eac54620cfb913e5ce36c25` (`# v0.36.0`; `severity: HIGH,CRITICAL`, `exit-code: 1`, `ignore-unfixed: true`). **Pinned by commit SHA, not by tag:** `CVE-2026-33634` / `GHSA-69fq-xp46-6x23` (critical) is a 2026-03-19 supply-chain compromise in which 76 of this action's 77 tags were force-pushed to credential-stealing malware. Upstream republished the legitimate tags `v`-prefixed, which is why the previously-referenced `@0.28.0` no longer resolves at all. The advisory marks SHA pins to commits before 2025-04-09 as affected; `ed142fd…` is 2026-04-22. Any future bump of this action must move the SHA deliberately and re-verify against `biochemistry-backend:ci` and `biochemistry-frontend:ci` inside the existing `docker-build` jobs, which now build with `load: true` so the images are scannable.
